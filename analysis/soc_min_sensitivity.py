...
def write_summary_markdown(rows: list[dict[str, object]], config_path: Path, output_dir: Path, path: Path) -> None:
    baseline_row = next((row for row in rows if float(row["soc_min_pct"]) == float(BASELINE_SOC_MIN_PCT)), None)
    intro_lines = [
        f"Config: `{config_path.relative_to(REPO_ROOT).as_posix()}`",
        f"Cases run: {len(rows)}",
        (
            "Baseline case at 20% minimum SOC: "
            f"{baseline_row['total_fuel_kg']:.3f} kg fuel, {baseline_row['total_starts']} starts, "
            f"terminal SOC {baseline_row['terminal_soc_pct']:.2f}%."
            if baseline_row
            else "Baseline case at 20% minimum SOC was not present in this sweep."
        ),
    ]
    write_markdown_summary(
        path,
        title="Minimum-SOC Sensitivity Summary",
        intro_lines=intro_lines,
        columns=[
            ("Minimum SOC [%]", "soc_min_pct"),
...
def main() -> None:
    ...
    output_dir, generated_configs_dir = create_output_dirs("soc_min", sensitivity_output_root(base_config))
