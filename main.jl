...
function write_results_csv(path, load, datetimes, gensets, battery, u, y, Pg, SFOC, lambda, P_ch, P_dis, E)
...
function main()
    ...
    # Must match STEP_MINUTES in data/generate_synthetic_profile.py
    dt_minutes = 15
    dt = dt_minutes / 60.0
    ...
    load_profile_path = joinpath(@__DIR__, "data", "load_profile.csv")
    isfile(load_profile_path) || error(
        "Missing $(load_profile_path). Run `python data/generate_synthetic_profile.py` first."
    )
    ...
    if termination_status(model) == OPTIMAL
        ...
        write_results_csv(csv_path, load, datetimes, gensets, battery, u, y, Pg, SFOC, lambda, P_ch, P_dis, E)
