...
function solve_rolling_horizon(cfg, load, datetimes, rolling_settings, show_solver_log)
    ...
    for r in T
        ...
        push!(solve_times, solve_time(components.model))
...
function main()
    ...
    rolling_settings.horizon_steps == 24 || error(
        "MVP rolling-horizon controller expects horizon_steps=24; got $(rolling_settings.horizon_steps)."
    )
    if rolling_settings.soc_strategy == "soft_band"
    ...
    params_dict = Dict(
        ...
        "terminal_conditions" => Dict(
            ...
            "rolling_local_terminal_constraint_enforced" => rolling_settings.soc_strategy == "terminal_reserve",
    ...
    if rolling_settings.soc_strategy == "soft_band"
        params_dict["soft_soc"] = Dict(
            ...
            "interpretation" => "Preferred rolling-horizon operating band with soft violations; not a terminal reserve constraint.",
        )
    end
