...
include("model_no_terminal_hard_soc_test.jl")
...
function main()
    config_arg = isempty(ARGS) ? joinpath("config", "operational_model_soc60_no_terminal_hard_soc.toml") : ARGS[1]
    ...
    isfile(load_profile_path) || error("Missing $(load_profile_path) referenced by $(config_path).")
    load, datetimes = read_load_profile(load_profile_path)

    model, u, y, Pg, mdot, lambda, P_ch, P_dis, E = build_model_no_terminal_hard_soc_test(
        gensets,
        load,
        battery,
    ...
    if termination_status(model) == OPTIMAL
        ...
        params_dict = Dict(
            "run" => Dict(
                ...
                "entry_point" => "experimental_models/main_no_terminal_hard_soc_test.jl",
                "model_file" => "experimental_models/model_no_terminal_hard_soc_test.jl",
            ),
            "solver" => Dict(
