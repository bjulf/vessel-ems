...
function build_model(gensets, load, battery)
    ...
    @constraint(model, E[length(T)+1] >= battery.SOC_ref * battery.E_max)
    ...
    # ── Objective: minimise total fuel flow + SOC deviation penalty ─────────
    @objective(model, Min,
    ...
    return model, u, Pg, SFOC, lambda, P_ch, P_dis, E, soc_dev
end
